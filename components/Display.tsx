  ...
  return (
    <main className="flex h-[70vh] flex-col items-center justify-between p-24 relative">
      ...
      <div className="absolute bottom-10">
        {isLoggedIn && !isGod && (
          <h2>
            User <span className="text-teal-500 font-bold">{userName}</span>{" "}
            logged in.
          </h2>
        )}
        {isGod && (
          <h2>
            <span className="text-teal-500 font-bold">{userName}</span> using{" "}
            <span className="font-bold">GodMode</span>
          </h2>
        )}
      </div>
